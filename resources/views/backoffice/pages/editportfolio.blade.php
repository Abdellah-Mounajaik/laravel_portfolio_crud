...
@section('back')
    <h1 class="text-center my-5">Modifier l'image</h1>
    <form method="POST" class="text-center container " enctype="multipart/form-data" action="{{route('portfolio.update', $portfolios->id)}}">
        @csrf
        @method('PUT')
        <a class="" href="{{route('back.portfolio')}}">Revenir à la page arrière</a><br>
    
        <label class="mt-4" for="image">Ajouter le lien d'une image :</label> <br>
